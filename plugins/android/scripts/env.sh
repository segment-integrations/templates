...
# android.json is generated by android-init.sh for Nix flake consumption
# No additional config loading needed here - all config is via environment variables
...
# Source all other Android scripts to make functions available
...
if [ -n "${ANDROID_SCRIPTS_DIR:-}" ]; then
  ...
  # avd.sh provides AVD setup and management functions
  if [ -f "${ANDROID_SCRIPTS_DIR}/avd.sh" ]; then
    . "${ANDROID_SCRIPTS_DIR}/avd.sh"
  fi

  # emulator.sh provides emulator lifecycle functions
  if [ -f "${ANDROID_SCRIPTS_DIR}/emulator.sh" ]; then
    . "${ANDROID_SCRIPTS_DIR}/emulator.sh"
  fi

  # deploy.sh provides app deployment functions
  if [ -f "${ANDROID_SCRIPTS_DIR}/deploy.sh" ]; then
    . "${ANDROID_SCRIPTS_DIR}/deploy.sh"
  fi
fi
